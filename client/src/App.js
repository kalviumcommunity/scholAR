...
const App = () => {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            let res;
            if (mode === 'cot') {
                res = await ask(question, dynamicMode);
            }
            else if (mode === 'zero') {
                const r = await askZeroShot(persona, question);
                res = { finalAnswer: r.response, explanation: 'Zero-shot response.' };
            }
            else if (mode === 'one') {
                const r = await askOneShot(question);
                res = { finalAnswer: r.response, explanation: 'One-shot primed response.' };
            }
            else {
                const r = await askMultiShot(question);
